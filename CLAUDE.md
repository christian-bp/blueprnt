...
- **Worktrees instead of feature branches.** We never push feature branches or review PRs, so a branch is only an isolation mechanism, and worktrees isolate better while allowing parallel work. Larger or parallel work happens in a worktree: `git worktree add ../<slug> -b <prefix>/<slug>` (backing branch uses the conventional prefixes: `feat/`, `fix/`, `chore/`, `docs/`, `refactor/`), then `bun install` inside it (worktrees share `.git` but not `node_modules`). Also copy `packages/backend/.env.local` and `apps/dashboard/.env.local` from the main checkout: they are gitignored but required for `bunx convex codegen` and local dev. Small single-concern changes commit directly on main.
- **Work lands on main as ONE squash commit** (`git merge --squash` from the main checkout), so main reads one commit per unit of work; the squash commit body is the record of what landed. Right after the merge, verify containment (`git diff <branch> main --stat` must be empty), then `git worktree remove ../<slug>` and delete the backing branch.
