...
- **Guide the user through every concept.** Ease of use and guidance are the product's primary goal. Every surface that introduces a domain term (criterion, assessment anchor, band, anchor role) explains it inline in plain language, normally a `HelpPopover` with a `dashboard.help.*` key next to the term. Flows state their preconditions in words ("the role needs a completed assessment") instead of silently hiding or disabling controls. Never stack more than one help popover on the same heading or row; place each concept's help where the concept is used (a second concept's help goes next to its own control). A new domain concept ships with its help text, in all locales, in the same commit.
