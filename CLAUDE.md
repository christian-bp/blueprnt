...
- **Feature branches land on main as ONE squash commit** (`git merge --squash`), so main reads one commit per feature. Keep the feature branch (or its PR) around when the granular history matters.
