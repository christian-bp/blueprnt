...
export const STATUS_VALUE_KEYS: Record<AuditStatus, string> = {
  ...
  // Reuses the working-conditions decision's own negative answer (the
  // materiality question's "no", which the dialog's choice card also wears)
  // rather than inventing new auditLog.values.* wording for a concept that
  // already has a real, dedicated surface.
  testedNotMaterial: "model.criteria.workingConditions.noCta",
}
