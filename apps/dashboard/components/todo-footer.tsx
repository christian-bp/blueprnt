...
// The staged exit a leaving row plays (docs/ui-animation.md #4): fade fast,
// then collapse the now-invisible box. Nothing visible can overflow a
...
//
// The BLOCK itself renders statically, a deliberate exception to the
// animate-transitions convention: AreaChrome is keyed by area, so every area
// switch remounts this component, and an enter animation here replayed on
// every page change, which read as noise rather than as a transition (the
// same reason the rows' own AnimatePresence suppresses initial animations).
// The ROWS still move: a group that FINISHES is not dropped on the spot; its
// row stays for one beat wearing the same CelebrationBurst the front page's
// cards throw, and only then collapses away. Finishing is "the group left
// buildTodo's output", never "the group left the two-row glance": a group
// pushed out by a higher-priority arrival has not been completed and gets no
// party.
export function TodoFooter() {
  ...
  return (
    <SidebarFooterBlock>
      ...
      {/* No gap on this column: an exiting row's share of the spacing is its
          own animated marginBottom, so the gap collapses with it
          (docs/ui-animation.md #3). */}
            ...
            >
              <CelebrationBurst active={finished}>
                ...
                >
                  ...
                  {/* Only quantity groups carry a count; a single act's
                      structural "1" is noise. A finished row's badge counts
                      down to nothing and springs out while the burst
                      plays. */}
