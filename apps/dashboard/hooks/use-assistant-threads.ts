...
// while the panel is open. A deliberately separate hook rather than folded
// into useAssistantChat: the list is bounded and indexed
// (ASSISTANT_THREAD_LIST_LIMIT, listThreads), so subscribing to it for as
// long as the panel stays open is cheap, but useAssistantChat is read by
// every assistant surface on every render, and that hook has no use for the
// full list at all. Keeping the two separate means the chat page never pays
// for a subscription only the panel's own thread-list node needs.
