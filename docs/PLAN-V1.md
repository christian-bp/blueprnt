...
- **i18n:** fem språk från start — **svenska (standard), engelska, norska (nb), danska, finska** — via `packages/i18n` (next-intl, typade nycklar, delat av web + dashboard). Sv/en seedas från ordlistornas i18n-tabeller; nb/da/fi är maskinöversatta utkast som ska granskas av modersmålstalare före lansering. Marknadssajten har locale i URL:en (sv utan prefix) med språkväxlings-dropdown i menyn; dashboarden har ingen locale i URL:en, språket är en inställning under account settings. Språkbyte sker alltid som full sidladdning (se CLAUDE.md).
