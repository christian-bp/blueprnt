...
Beslutet ovan står fast, orden har bytt namn. Det som här heter **Nivå** (individens senioritet inom rollens track) heter i dag **Senioritet** (kod `seniority`, fältet `personAssignments.seniority`, konstanten `TRACK_SENIORITIES`, valideringen `isValidSeniorityForTrack`, tidigare `isValidLevelForTrack`), och det som här heter **Band** (rollens beräknade tyngd) heter i dag **Nivå** (kod `level`, Nivå 1 är högst). Notens sista mening är dessutom inaktuell i sak: `updateRole` blockerar inte längre ett track-byte (`errors.roleTrackChangeBlocked` är borttagen); i stället återställs berörda individers senioritet vid bytet och bekräftas på nytt (`senioritiesReset`). Texten ovan skrivs inte om: läs den med den nyckeln. Se [ADR-0014](./0014-terminologi-niva-senioritet-steg.md).
