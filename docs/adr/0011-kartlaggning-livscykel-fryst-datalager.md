...
3. **Åtkomst- och exportloggning.** Utöver dagens invariant ("varje tillståndsändrande mutation skriver en revisionsrad") loggas även **visningar och exporter** av en kartläggning. Detta är en ny, läs-orienterad revisionsdimension för kartläggningsdata; den ska inte tillämpas brett på övriga ytor.
   ...
   **Uppdatering (2026-07-13):** visningsloggning byggs inte i det första snapshot-lagret (denna ADR). v1 av metodstödet kräver bara en ändringslogg (vem ändrade vad), och den täcks redan av den befintliga domänrevisionsloggen. Kravets verkliga värde ligger vid exportgränsen, inte vid varje visning: det frysta detaljvyn visar samma persondata som redan är obevakat synlig i de levande People/lön-vyerna, så en visningslogg per sida tillför ingen ny skyddsnivå idag. Åtkomst- och exportloggning skjuts därför upp till export-/rapportmodulen (guidens Modul 8) och loggas där, vid exportgränsen där data lämnar systemet. Beslutspunkt 3 ovan står kvar som den framtida målbilden; den implementeras i exportslicen, inte i denna.
