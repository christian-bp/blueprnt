...
9. **§4.1 standard text lives in the help layer, not as standing prose** (decision 13, 2026-08-23): the masterdokument asks every step to open with VARFOR/VAD/VAD HANDER blocks; the framing-prose law (CLAUDE.md, 2026-08-21) rules standing explainer sentences a defect. The §4.1 content is carried by titles, HelpMorphButton bodies, and preconditions-in-words instead (the stage eyebrows named there were retired 2026-08-25; see deviation 10). The pedagogy is kept; the prose form is not.
10. **Stage headings are dropped; the firewall is structural** (§4.2.2, §17.5): the masterdokument asks each surface to name its stage, and METODBYGGNAD/ROLLBEDOMNING shipped as scanned eyebrows until the owner retired them (ruling 2026-08-25) because the to-do guidance and the surfaces' own identities already carry the orientation. What deviation 10 still holds is the part that was never signage: the assessment route links to no builder or results surface, and the rating wire carries no weighting at all, so the separation is enforced by route isolation and by what the client is sent rather than by a label. We do not hard-block navigation for the HR-only audience (extends deviation 2).
...
- A verified absence of builder/results links from the rate route (deviation 10). The stage eyebrows that shipped alongside it were retired 2026-08-25; the link isolation is the firewall.
