...
- One record per person per compensation period: `orgId`, `personId`, `effectiveAt` (the date the salary takes effect), `payYear` (`Löneår`, the analysis-period tag), `source` (`"import" | "manual"`), `basicMonthly` (`Månadslön`, the Art. 9 basic-salary component, parsed from `"94 500 kr"`, stored distinctly), `currency` (`Valuta`, trimmed), an **extensible `components` array** of `{ kind, monthlyAmount }` for every non-basic component (variable/bonus `Målbonus`, benefit-in-kind `Tjänstebil`, fixed supplements, allowances, equity, ...), with `kind` from a shared `PAY_COMPONENT_KINDS` set so any company's comp maps in. `createdAt`.
- **Total comp is the comparison metric.** The pay-gap comparison uses **total compensation** = `basicMonthly + sum(components.monthlyAmount)`, FTE-adjusted, NOT basic alone (product decision). The Art. 9 basic-vs-variable split is still derived and reported. Total comp is DERIVED, never stored (like score/band); a shared `totalMonthlyComp` helper computes it.
- **History, not overwrite.** A raise, a correction, a re-import, or a manual entry appends a new record; prior records are retained. The current salary is the latest `effectiveAt`; a report as-of a date uses the record effective at that date. An erasure hard-deletes all of a person's records (§9).
- **No stored derived figures.** The gap engine computes the FTE-adjusted total comp live and freezes it in the report run; only the raw `basicMonthly` + `components` are stored.
...
**Normalization + metric:** the pay-gap comparison metric is FTE-adjusted **total comp** = `(basicMonthly + sum(components)) / (ftePercent/100)`. The basic-vs-variable split is retained for Art. 9 reporting. Derived live, frozen per run.
