...
// gestureId is the CLIENT's correlation id for one user gesture that spans more
...
// correlation ids may not share one name.
// Deliberately UNINDEXED. Grouping is a render concern over the page already
// fetched, never a query, so an index would only pay for a "show me this
// gesture" retrieval surface, which no product surface asks for. A gesture
// larger than one page therefore renders as one part per page; see
// apps/dashboard/lib/audit-stories.ts for what that means and why it is
// deliberate.
